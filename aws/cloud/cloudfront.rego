...
default cf_not_wafv2 = true

cf_not_wafv2 = false {
    Y := input.TEST_CF[_]
    Y.Distribution.DistributionConfig.WebACLId == ""
}

cf_not_wafv2 = false {
    Y := input.TEST_CF[_]
    not Y.Distribution.DistributionConfig.WebACLId
}

cf_not_wafv2 = false {
    Y := input.TEST_CF[_]
    Y.Distribution.DistributionConfig.WebACLId == null
}
...
cf_not_wafv2 = false {
    ...
    X := input.TEST_WAF[_]
    WebACL := X.WebACLs[_]
    Y.Distribution.DistributionConfig.WebACLId == WebACL.WebACLId
}
...
cf_not_wafv2_err = "Ensure AWS CloudFront is configured with AWS Web Application Firewall v2 (AWS WAFv2)." {
    not cf_not_wafv2
}

cf_not_wafv2_metadata := {
    "Policy Code": "PR-AWS-CLD-CF-011",
    "Type": "cloud",
    "Product": "AWS",
    "Language": "AWS Cloud",
    "Policy Title": "Ensure AWS CloudFront is configured with AWS Web Application Firewall v2 (AWS WAFv2).",
    "Policy Description": "It identifies AWS CloudFront which is not configured with AWS Web Application Firewall v2 (AWS WAFv2). As a best practice, configure the AWS WAFv2 service on the CloudFront to protect against application-layer attacks. To block malicious requests to your CloudFront, define the block criteria in the WAFv2 web access control list (web ACL).",
    "Resource Type": "",
    "Policy Help URL": "",
    "Resource Help URL": "https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/cloudfront.html#CloudFront.Client.get_distribution"
}