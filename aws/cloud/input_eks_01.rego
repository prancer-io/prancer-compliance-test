...
rulepass = true {
    ...
    input.json.cluster.resourcesVpcConfig.endpointPrivateAccess=true
    input.json.cluster.resourcesVpcConfig.endpointPublicAccess=false
}
...
metadata := {
    ...
    "Policy Description": "When you create a new cluster, Amazon EKS creates an endpoint for the managed Kubernetes API server that you use to communicate with your cluster (using Kubernetes management tools such as kubectl). By default, this API server endpoint is public to the internet, and access to the API server is secured using a combination of AWS Identity and Access Management (IAM) and native Kubernetes Role Based Access Control (RBAC)._x005F_x000D_ _x005F_x000D_ This policy checks your Kubernetes cluster endpoint access and triggers an alert if publicly enabled.",
    "Resource Type": "",
    "Policy Help URL": "",
