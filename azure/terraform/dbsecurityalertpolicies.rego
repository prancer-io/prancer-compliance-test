...
# Check if azurerm_mssql_server_security_alert_policy resources exist
azurerm_mssql_server_security_alert_policy_exists {
    existence.azure_resource_exists("azurerm_mssql_server_security_alert_policy")
}
...
default sql_dbsec_threat_off = null

azure_attribute_absence["sql_dbsec_threat_off"] {
    count([c | input.resources[_].type == "azurerm_mssql_server_security_alert_policy"; c := 1]) == 0
}
...
azure_issue["sql_dbsec_threat_off"] {
    resource := input.resources[_]
...
sql_dbsec_threat_off {
    lower(input.resources[_].type) == "azurerm_sql_server"
    not azure_attribute_absence["sql_dbsec_threat_off"]
    not azure_issue["sql_dbsec_threat_off"]
}

sql_dbsec_threat_off = false {
    lower(input.resources[_].type) == "azurerm_sql_server"
    azure_attribute_absence["sql_dbsec_threat_off"]
}
...
sql_dbsec_threat_off_err = "azurerm_mssql_server_security_alert_policy resource or its property 'state' is missing from the resource. Set the value to 'Enabled' after property addition." {
    lower(input.resources[_].type) == "azurerm_sql_server"
    azure_attribute_absence["sql_dbsec_threat_off"]
} else = "SQL Database Server security alert policy is currently not enabled" {
    lower(input.resources[_].type) == "azurerm_sql_server"
...
sql_dbsec_threat_off_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-008",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "SQL Database Server should have security alert policies enabled",
...
default sql_dbsec_threat_retention = null

azure_attribute_absence["sql_dbsec_threat_retention"] {
    count([c | input.resources[_].type == "azurerm_mssql_server_security_alert_policy"; c := 1]) == 0
}
...
azure_issue["sql_dbsec_threat_retention"] {
    resource := input.resources[_]
    lower(resource.type) == "azurerm_sql_server"
    count([c | r := input.resources[_];
              r.type == "azurerm_mssql_server_security_alert_policy";
...
sql_dbsec_threat_retention {
    lower(input.resources[_].type) == "azurerm_sql_server"
    not azure_attribute_absence["sql_dbsec_threat_retention"]
    not azure_issue["sql_dbsec_threat_retention"]
}

sql_dbsec_threat_retention = false {
    lower(input.resources[_].type) == "azurerm_sql_server"
    azure_attribute_absence["sql_dbsec_threat_retention"]
}
...
sql_dbsec_threat_retention_err = "azurerm_mssql_server_security_alert_policy resource or its property 'retention_days' need to be exist. Its missing from the resource. Please set the value to '90' after property addition." {
    lower(input.resources[_].type) == "azurerm_sql_server"
    azure_attribute_absence["sql_dbsec_threat_retention"]
} else = "Azure SQL Database Server security alert policies thread retention is currently not configured for 90 days or more" {
    lower(input.resources[_].type) == "azurerm_sql_server"
...
sql_dbsec_threat_retention_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-009",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "Azure SQL Database Server security alert policies thread retention should be configured for 90 days or more",
...
default sql_dbsec_threat_email = null

azure_attribute_absence["sql_dbsec_threat_email"] {
    count([c | input.resources[_].type == "azurerm_mssql_server_security_alert_policy"; c := 1]) == 0
}
...
azure_issue["sql_dbsec_threat_email"] {
    resource := input.resources[_]
    lower(resource.type) == "azurerm_sql_server"
    count([c | r := input.resources[_];
              r.type == "azurerm_mssql_server_security_alert_policy";
...
sql_dbsec_threat_email {
    lower(input.resources[_].type) == "azurerm_sql_server"
    not azure_attribute_absence["sql_dbsec_threat_email"]
    not azure_issue["sql_dbsec_threat_email"]
}

sql_dbsec_threat_email = false {
    lower(input.resources[_].type) == "azurerm_sql_server"
    azure_attribute_absence["sql_dbsec_threat_email"]
}
...
sql_dbsec_threat_email_err = "azurerm_mssql_server_security_alert_policy resource or its property 'email_addresses' need to be exist. Those are missing from the resource." {
    lower(input.resources[_].type) == "azurerm_sql_server"
    azure_attribute_absence["sql_dbsec_threat_email"]
} else = "Azure SQL Database Server security alert policy is currently not configured to sent alert to the configured email addresses" {
    lower(input.resources[_].type) == "azurerm_sql_server"
...
sql_dbsec_threat_email_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-010",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "Azure SQL Database Server Security Alert Policy should be configured to send alert to the configured email addresses",
...
default sql_dbsec_threat_alert = null

azure_attribute_absence["sql_dbsec_threat_alert"] {
    count([c | input.resources[_].type == "azurerm_mssql_server_security_alert_policy"; c := 1]) == 0
}
...
azure_issue["sql_dbsec_threat_alert"] {
    resource := input.resources[_]
    lower(resource.type) == "azurerm_sql_server"
    count([c | r := input.resources[_];
              r.type == "azurerm_mssql_server_security_alert_policy";
...
sql_dbsec_threat_alert {
    lower(input.resources[_].type) == "azurerm_sql_server"
    not azure_attribute_absence["sql_dbsec_threat_alert"]
    not azure_issue["sql_dbsec_threat_alert"]
}
...
sql_dbsec_threat_alert = false {
    lower(input.resources[_].type) == "azurerm_sql_server"
    not azure_attribute_absence["sql_dbsec_threat_alert"]
    azure_issue["sql_dbsec_threat_alert"]
}

sql_dbsec_threat_alert_err = "Azure SQL Server threat detection alerts not enabled for all threat types" {
    lower(input.resources[_].type) == "azurerm_sql_server"
    not azure_attribute_absence["sql_dbsec_threat_alert"]
    azure_issue["sql_dbsec_threat_alert"]
}

sql_dbsec_threat_alert_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-013",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "Azure SQL Server threat detection alerts should be enabled for all threat types",
...
default mssql_alert = null

azure_attribute_absence["mssql_alert"] {
    count([c | input.resources[_].type == "azurerm_mssql_server_security_alert_policy"; c := 1]) == 0
}
...
mssql_alert {
    lower(input.resources[_].type) == "azurerm_mssql_server"
    not azure_attribute_absence["mssql_alert"]
    not azure_issue["mssql_alert"]
}
...
mssql_alert_err = "Threat Detection alert currently is not configured to sent notification to the sql server account administrators" {
    lower(input.resources[_].type) == "azurerm_mssql_server"
    azure_issue["mssql_alert"]
...
sql_alert_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-015",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "Threat Detection alert should be configured to sent notification to the sql server account administrators",
