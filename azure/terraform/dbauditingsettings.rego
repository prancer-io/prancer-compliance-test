...
#    resource := input.resources[_]
#    lower(resource.type) == "azurerm_sql_database"
#    count([c | input.resources[_].type == "azurerm_mssql_database_extended_auditing_policy"; 
#           c := 1]) == 0
#}
...
sql_db_log_audit {
    lower(input.resources[_].type) == "azurerm_sql_database"
...
sql_db_log_audit_err = "azurerm_mssql_database_extended_auditing_policy resource is missing from template or its not linked with target azurerm_sql_database" {
    lower(input.resources[_].type) == "azurerm_sql_database"
    azure_attribute_absence["sql_db_log_audit"]
} else = "Auditing for SQL database is not enabled" {
    lower(input.resources[_].type) == "azurerm_sql_database"
...
sql_db_log_audit_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-036",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "Auditing for SQL database should be enabled",
...
sql_db_log_retention_err = "azurerm_mssql_database_extended_auditing_policy resource is missing or its property 'retention_in_days' is missing" {
    lower(input.resources[_].type) == "azurerm_sql_database"
    azure_attribute_absence["sql_db_log_retention"]
} else = "Azure SQL Database with Auditing Retention is not equal or more then 90 days" {
    lower(input.resources[_].type) == "azurerm_sql_database"
...
sql_db_log_retention_metadata := {
    "Policy Code": "PR-AZR-TRF-SQL-037",
    "Type": "IaC",
    "Product": "AZR",
    "Language": "Terraform",
    "Policy Title": "Ensure Azure SQL Database Auditing Retention is minimum 90 days or more",
